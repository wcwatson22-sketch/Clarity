...
export class AppComponent {
  ...
  constructor() {
    ...
    // Bottom bar: the free tabs. The Premium button is appended in the template.
    // Learn is NOT part of the authenticated website — it only appears in the
    // native app's bottom nav, pointing at the in-app Learn experience.
    const find = (p: string) => this.navItems.find(i => i.path === p)!;
    const bottom = [find('/dashboard'), find('/cash-flow')];
    if (Capacitor.isNativePlatform()) {
      const learnIcon = this.sanitizer.bypassSecurityTrustHtml(
        `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>`
      );
      bottom.push({ path: '/app-learn', label: 'Learn', icon: learnIcon });
    }
    bottom.push(find('/settings'));
    this.primaryNavItems = bottom;
